...
# Seed
np.random.seed(0)
key = jax.random.PRNGKey(0)  # You can set the seed for reproducibility
...
# === Sweep parameters ===
k_theta_range = np.array([0.000005,0.00005,0.0005,0.005,0.05,0.5])	# bend resistance
k_tilt_range = np.array([0.000005,0.00005,0.0005,0.005,0.05,0.5])	# tilt resistance
V_range = np.array([1])
...
for V in V_range:
	...
	for k_tilt in k_tilt_range:
		for k_theta in k_theta_range:
			...
			# --- Open output file ---
			filename = f"Data/data_kl_{k_lateral}_ko_{k_actin_outer}_ka_{k_theta}_kt_{k_tilt}_Ri_{R0_in}_beta_{beta}_N_{Nlateral}_d_{disorder}_{word}_V{V:03d}.txt"
			with open(filename, "w") as file:
				...
				for gstep in range(Niterations):
					...
					print(f'\t{factor:.2f}/{growth:.2f}:\t Frms = {Frms:.3g} in {niters} steps.')
					...
					P_list = []
					dA = 0.01
					A_current = np.pi*Rorganoid**2
					for delta_A in [-dA, dA]:
						R_adjusted = jnp.sqrt((A_current + delta_A) / jnp.pi)
						adjusted_r = jv.affine_deformation(r, Rorganoid, R_adjusted)

						r_p, Frms, niters = jv.run_minimization(combined_force_fn, adjusted_r, l0_list, invmass, constrained, i_fix, R_adjusted, max_frms_thresh=max_Frms, max_num_steps=max_iter)
						E_adjusted = combined_energy_fn(r_p, l0_list=l0_list)
						P_list.append(E_adjusted)
					P = (P_list[1] - P_list[0]) / (2 * dA)
