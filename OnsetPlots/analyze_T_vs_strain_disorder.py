...
logC_vals = np.log10(kt_range)
norm = matplotlib.colors.Normalize(vmin=np.floor(logC_vals.min())-1, vmax=np.ceil(logC_vals.max()))
colors = cmap_grey(norm(logC_vals))
...
ax2.set_ylim([1e-8,5e0])
ax2.set_zorder(ax.get_zorder() - 1)
ax.patch.set_visible(False)  # make ax2 background transparent
...
redcolor = cmap(0.75)
greycolor = cmap_grey(0.75)
ax.spines['left'].set_color(redcolor)
ax.tick_params(axis='y', colors=redcolor)
ax.set_ylabel(r'$\langle T \rangle$', color = redcolor)

ax2.spines['right'].set_color(greycolor)
ax2.tick_params(axis='y', colors=greycolor)
ax2.set_ylabel(r'$\langle (\theta - \theta_0)^2 \rangle$', color = greycolor)

plotlib.set_position(ax,x=0.26,y=0.20,width=0.48,height=0.75)
# Save the figure

fig.savefig('./Plots/T_vstrain_Sim_disorder.pdf',dpi=600,transparent=True)
